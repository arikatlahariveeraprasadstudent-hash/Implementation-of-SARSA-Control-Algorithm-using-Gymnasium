...
The agent's task is to learn an optimal policy to navigate from the start to the goal while avoiding holes, maximizing its cumulative reward despite the slippery surface.
...
# Final Q-table:
<img width="234" height="343" alt="image" src="https://github.com/user-attachments/assets/cb0392ab-e200-45a8-b27f-256cfe31d185" />



# Estimated State-Value Function:

<img width="258" height="115" alt="image" src="https://github.com/user-attachments/assets/aa8566e9-4a97-4802-9546-a9d247f7265f" />



# Learned Policy:
<img width="179" height="111" alt="image" src="https://github.com/user-attachments/assets/e06f0c05-b609-45a7-a95e-b859d1c00fb9" />


# Average reward over last 1000 episodes: 
<img width="397" height="32" alt="image" src="https://github.com/user-attachments/assets/13cdcb5e-0b04-423c-b35a-e345f0db9f32" />
...
The agent learned an action-value function using the SARSA update rule and an epsilon-greedy policy. The learned Q-table was used to obtain the state-value function and the best action for each state.
...
From this experiment, it can be inferred that **SARSA is an on-policy reinforcement learning algorithm that learns from the actions actually selected by the agent**.

The Q-table initially contains zero values because the agent has no prior knowledge of the environment. During training, the Q-values are gradually updated using the rewards received from the environment.

The epsilon-greedy strategy allows the agent to balance **exploration** and **exploitation**. At the beginning of training, a high epsilon encourages exploration. As epsilon decreases, the agent increasingly selects actions with higher Q-values.

The final Q-table represents the learned action values, while the state-value function represents the best estimated value of each state. The learned policy selects the action with the highest Q-value for each state.

Therefore, the experiment demonstrates how **SARSA can learn an effective control policy through repeated interaction with an environment without being explicitly given the correct path to the goal**.
